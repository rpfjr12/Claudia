...
def run_strict_mode(findings):
    """
    Wrapper that runs enforce_strict_mode and prints a summary.
    """
    print("[strict_mode] Applying strict mode filters...")
    result = enforce_strict_mode(findings)
    print(f"[strict_mode] {len(result)} findings passed strict mode (from {len(findings)})")
    return result
